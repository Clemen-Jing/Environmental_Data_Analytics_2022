 ...
 * Managing how code/products is displayed
- Add formatted text along with our code
- All text based - good for reproducibility
Both *and -work but remeber there shoud be a vacant line before the list.
...
file <- data.frame("file name"<- c("EPAair_O3_NC2018_raw" , "EPAair_O3_NC2019_raw", "EPAair_PM25_NC2018_raw", "EPAair_PM25_NC2019_raw"),
                   "file description" <- c("9737 rows, 20 columns"," 10592 rows, 20 columns","8983 rows, 20 columns", "8581 rows, 20 columns"))

kable(file, format = "simple", col.names = c('File Name', 'File Description'),
      caption = "Example Dataset information")
```
...
```{r Install Knitr}
library(knitr)
kable(cars, caption = "My cars table")
```
...
water <- read.csv('~/Environmental_Data_Analytics_2022/Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv', stringsAsFactors = TRUE)
...
```{r include=FALSE}
water.tn <- water.processed %>%
  group_by(lakename) %>%
  summarise(mean_tn = mean(tn_ug), 
            minimum_tn = min(tn_ug),
            maximum_tn = max(tn_ug),
            sd_tn = sd(tn_ug))
  
water.tp <- water.processed %>%
  group_by(lakename) %>%
  summarise(mean_tp = mean(tp_ug), 
            minimum_tp = min(tp_ug),
            maximum_tp = max(tp_ug),
            sd_tp = sd(tp_ug))
```
...
```{r echo=FALSE}
kable(water.tn, caption = "Total Nitrogen of Lakes")
kable(water.tp, caption = "Total Phosphorus of Lakes")
```
...
```{r echo=FALSE}
#| fig.align = 'left', fig.cap = 'Total Nitrogen Level of Lakes 1991-1999'
ggplot(water.processed, aes(x = sampledate, y = tn_ug, color = lakename)) +
  geom_point() +
...
```{r echo=FALSE}
#| fig.align = 'left' , fig.cap = 'Total Phosphorus Level of Lakes 1991-1999'
ggplot(water.processed, aes(x = sampledate, y = tp_ug, color = lakename)) +
  geom_point() +
  geom_smooth(method = lm) +
  labs(
...
> Based on Table 1 and 2, we can see that the nutrient level of the lakes varied a lot. Out of the total 8 lakes that were collected data from, East Long lake, West long lake, Central long lake, and Peters lake have a higher mean of nutrients from 1991 to 1999. However, from the two figures we can see that all lakes presented similar level of nutrients, both Nitrogen and Phosphorus, at 1991 while these four specific lakes experienced a much more significant growth in the nutrient level as the date approaches 1999. By the time of 1999, East Long lake is having a significantly higher level of both Nitrogen and Phosphorus compared with other lakes.
