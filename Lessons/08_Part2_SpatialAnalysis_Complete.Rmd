...
svi2018_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>% #as factor or character rather than numbers because the numbers represent the mark of the counties, not value.
...
leaflet() %>% addTiles() %>% 
  ...
  addPolygons(data=huc8_sf,weight=1) #it cannot be shown in UTM crs
```
