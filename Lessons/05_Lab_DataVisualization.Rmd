...
ggplot(data=data.frame(x=0,y=0))+
  geom_point(aes(x=x,y=y)) +
  geom_text(x=0,y=0.025,label=expression(paste("Mean is ", digree,",", R[5],'=0.6')))
print(Plot_text2)
```
...
Plot1 <- ggplot(data = PeterPaul.chem.nutrients)+
  geom_point(aes(x = daynum, y = temperature_C, colours = year4)) +
...
Plot2 <-
  ...
  geom_point(aes(x = sampledate, y = depth), size = 0.5)+
